...
export function PressureDN1350Title() {
  return (
...
export default function PressureDN1350({
  hideTitle = false,
  className,
  upstream: liveUpstream,
  downstream: liveDownstream,
}: PressureDN1350Props) {
  const [simUpstream, setSimUpstream] = useState(8.0);
  const [simDownstream, setSimDownstream] = useState(7.5);
  ...
  const pressureUpstream = liveUpstream ?? simUpstream;
  const pressureDownstream = liveDownstream ?? simDownstream;
